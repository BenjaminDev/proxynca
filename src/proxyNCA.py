...
class DML(pl.LightningModule):
    ...
    def validation_epoch_end(self, outputs):
        ...
        for i, example_result in enumerate(top_k_indices[[randint(0,max_idx) for _ in range(0,5)]]):
            ...
            image_dict[f"global step {self.global_step} example: {i}"].extend([wandb.Image(Image.open(self.val_im_paths[val_indexes[idx]]), caption=f"retrial: rank({rank}) image_label {self.val_dataset.get_label(val_indexes[idx])}") for rank, idx in enumerate(example_result[1:])])
